...
$resultado=$conexion->query($sql);
 if(!empty($_POST)){
         $subtemas = mysqli_real_escape_string($conexion,$_POST['subtemas']);
         $subparrafo = mysqli_real_escape_string($conexion,$_POST['subparrafo']);
         $no_subparrafo = mysqli_real_escape_string($conexion,$_POST['no_subparrafo']);
         $sqlsubparrafo = "INSERT INTO subparrafo(subparrafo,no_subparrafo,idsubtema) VALUES ('$subparrafo','$no_subparrafo','$subtemas')";
         $resultadosubparrafo=$conexion->query($sqlsubparrafo);

         if ($resultadosubparrafo>0) {
             echo "<script>
 alert('Registro de subparrafo exitoso');
 window.location='agregar-subparrafo.php';</script>";
         }else{
             echo "<script>
 alert('Error al registrar subparrafo');
 window.location='agregar-subparrafo.php';</script>";
         }
     }
...
<div class="home-content">
      ...
      <span class="titulo"><h2>Agrega información a un subtema de tu preferencia</h2z></span>
</div>
<div class="container f-c texto">
  ...
  <input type="text" name="subparrafo" class="form-control" placeholder="Subpárrafo" aria-label="Username" aria-describedby="basic-addon1" pattern="[A-Za-zÁÉÍÓÚñáéíóú0-9\s.,!?()_-]{10,5000}" required>
...
<div class="input-group mb-3">
  <div class="input-group-prepend">
    <span class="input-group-text" id="basic-addon1">Ingrese el numero de subpárrafo (su posición en el tema seleccionado), tome en cuenta que 1 es el primero, dos el segundo...</span>
  </div>
  <input type="number" name="no_subparrafo" class="form-control" class="form-control" placeholder="no subpárrafo" aria-label="Username" aria-describedby="basic-addon1" pattern="\d{1,4}" required>
  <div class="invalid-feedback">
    El valor introducido no es valido, por favor introduce un valor valido
  </div>
</div>
